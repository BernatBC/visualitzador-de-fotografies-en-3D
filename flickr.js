(function () {
    ...
    window.App = {
        ...
        select: function (photo) {
            ...
            if (photo) {
                this.viewer.world.setItemIndex(
                    photo.tiledImage,
                    this.viewer.world.getItemCount() - 1
                );
        ...
        frame: function () {
            ...
            _.each(this.photos, function (photo) {
                if (photo.diff.x || photo.diff.y) {
                    ...
                    var pos = photo.getPosition().plus(photo.diff.times(0.2));
            ...
            if (this.selectedPhoto) {
                ...
                _.each(this.selectedPhoto.tags, function (tag) {
                    if (tag.diff.x || tag.diff.y) {
                        var pos = tag.getPosition().plus(tag.diff.times(0.2));
        ...
        flickrRequest: function (config) {
            ...
            var url =
                "https://api.flickr.com/services/rest/?method=flickr." +
                config.method +
                "&api_key=" +
                apiKey +
                "&format=json";
        ...
        updateButtons: function () {
            ...
            if (
                (intersectionArea < viewArea * 0.5 && intersectionArea < photoArea * 0.33) ||
                photoArea < viewArea * 0.1
            ) {
                this.select(null);
            }
        },
                ...
                _.each(photo.tags, function (v, i) {
                    v.setPosition(
                        new OpenSeadragon.Point(
                            tl.x + Math.random() * box.width,
                            tl.y + Math.random() * box.height
                        )
                    );
                    v.show();
                });
